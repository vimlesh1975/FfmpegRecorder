...
Partial Public Class RecorderHostForm
    ...
    Private Async Sub StartDeckLinkRouting(camName As String, recorderControl As RecorderControl)
        ...
        Dim previewFps = 8
        Dim filterGraph = $"[0:v]scale=960:540:force_original_aspect_ratio=decrease,pad=960:540:(ow-iw)/2:(oh-ih)/2,fps={previewFps},format=yuv420p[video];[0:a]aresample=48000,pan=stereo|c0=c0|c1=c1[input_a];[input_a]asplit=2[left_meter_src][right_meter_src];[left_meter_src]pan=mono|c0=c0,showvolume=r={previewFps}:w=80:h=540:f=0.92:b=2:t=0:v=1:dm=1:o=v:ds=log:p=0.18:m=r[left_bar_src];[left_bar_src]scale=20:540,format=yuv420p[left_bar];[right_meter_src]pan=mono|c0=c1,showvolume=r={previewFps}:w=80:h=540:f=0.92:b=2:t=0:v=1:dm=1:o=v:ds=log:p=0.18:m=r[right_bar_src];[right_bar_src]scale=20:540,format=yuv420p[right_bar];[left_bar][video][right_bar]hstack=inputs=3[preview]"
